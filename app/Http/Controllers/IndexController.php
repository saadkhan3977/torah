...
class IndexController extends Controller
{
    ...
    public function index()
    {
        ...
        $menu = 'Laravel';
        return view("web.welcome")->with('title', 'Laravel')->with(compact('menu'));
        // ->with("title", "Home")
        //     ->with(compact(
        //         "banner",
        //         "content_1",
    ...
    {
        return view('web.services')->with('title', "Our Service");
    }
    public function blog()
    {
        // $menu = 'Our Blog';
        return view('web.blog')->with('title', "Our Blog");
    }
    public function car_fleet()
    {
        // $menu = 'Our Blog';
        return view('web.car_fleet')->with('title', "Car Fleet");
    }
    ...
    public function shop()
    {
        ...
        $setting = setting::find(1);
        return view('web.shop')->with('title', "Shop")->with(compact('inner_banner', 'setting'));
    }
    ...
    public function product()
    {
        ...
        return view('web.product')->with('title', 'Products')->with(compact('menu', 'keywords', 'description', 'inner_banner', 'products', 'category'));
    }
    public function product_details($slug = '')
    {
        ...
        $details = products::where("is_active", 1)->where('slug', $slug)->first();
        $related_products = products::where('category_id', $details->category_id)->where('is_active', 1)->orderBy('id', 'DESC')->get();
        // dd($related_products);
        if ($details->slug != $slug) {
            return redirect()->back()->with('error', 'No product details Found');
        }
        return view('web.product_details')->with('title', 'Product Details')
